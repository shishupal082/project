...
ApiHandler.extend({
    ...
    handlePageLoad: function(dbDataApis, callback) {
        ...
        var tableData;
        if (status === "completed") {
            status = CommonDataHandler.getData("dbViewDataLoadStatus");
            if (status === "not-started") {
                CommonDataHandler.setData("dbViewDataLoadStatus", "in-progress");
                this._loadDBViewData(dbDataApis, function(request) {
                    CommonDataHandler.setData("dbViewDataLoadStatus", "completed");
